...
export function getStatusBadge(status: string) {
  switch (status) {
    ...
    default:
      return (
        <span className="px-2 py-0.5 rounded-full text-[10px] font-mono font-semibold bg-subtle-2 text-accent border border-line">
          In Progress ⚡
        </span>
      );
  }
  ...
  return (
    ...
    >
      ...
      <div className="space-y-2.5">
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-center space-x-3 min-w-0 flex-1">
            <div className="w-10 h-10 shrink-0 rounded-xl bg-subtle border border-line flex items-center justify-center text-xl shadow-2xs group-hover:scale-105 transition-transform select-none">
              {displayedIcon}
            </div>
            <div className="min-w-0 flex-1">
              <h3 className="font-serif text-lg font-bold text-ink leading-tight truncate group-hover:text-accent transition-colors">
                {goal.title}
              </h3>
              {goal.targetDate && (
                <span className="text-[11px] font-mono text-faint flex items-center gap-1 mt-1 truncate">
      ...
      <div className="pt-3 border-t border-line space-y-2">
        ...
        {/* Card Footer Action */}
        <div className="flex items-center justify-between pt-0.5 text-[11px] text-faint">
          ...
          {completedMilestones < totalMilestones && (
            <button
              ...
              className="text-[10px] font-mono px-2 py-0.5 rounded bg-subtle-2 hover:bg-line text-ink transition-colors cursor-pointer"
