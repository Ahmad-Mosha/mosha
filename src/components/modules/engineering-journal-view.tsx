  ...
  const getCategoryBadge = (cat: string) => {
    switch (cat) {
      ...
      case "architectural_decision":
        return (
          <span className="px-2 py-0.5 rounded text-meta font-mono font-semibold bg-info-tint text-info border border-info/35">
            Architecture RFC
          </span>
        );
      default:
        return (
          <span className="px-2 py-0.5 rounded text-meta font-mono font-semibold bg-shipped-tint text-shipped border border-shipped/35">
            Reflection
          </span>
        );
    }
        ...
        >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-label">
            ...
            <div className="space-y-1">
              <label className="font-mono text-meta uppercase text-faint">Category</label>
              <Select
                value={category}
          ...
          <div className="space-y-1 text-label">
            <label className="font-mono text-meta uppercase text-faint">Content *</label>
            <textarea
              rows={4}
              required
              value={content}
          ...
          entries.map((entry: any) => (
            ...
            >
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-line pb-3">
                ...
                <span className="text-meta font-mono text-faint flex items-center gap-1">
                  <Calendar className="w-3 h-3" />
                  {entry.date}
                </span>
              </div>

              <p className="text-label sm:text-body text-muted leading-relaxed whitespace-pre-wrap">
                {entry.content}
              </p>

              <div className="flex items-center gap-2 pt-2 text-meta font-mono text-faint">
