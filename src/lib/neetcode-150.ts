/**
 * The NeetCode 150, as reference data.
 *
 * This is a curriculum, not user content — it never changes and is not stored
 * in Convex. Progress lives in the `problems` table, joined to these entries
 * by `slug`. Keeping the list here means the roadmap renders instantly and is
 * complete from the first run, before a single problem has been attempted.
 *
 * Order within a pattern is NeetCode's own teaching order: earlier problems
 * build the intuition the later ones assume.
 */
...
/** [slug, title, difficulty] — kept terse so the whole list stays readable. */
type Entry = [string, string, Difficulty];

const GROUPS: { pattern: string; items: Entry[] }[] = [
  { pattern: "Arrays & Hashing", items: [
  ...
  { pattern: "Heap / Priority Queue", items: [
  ...
  { pattern: "Graphs", items: [
    ...
    ["number-of-connected-components-in-an-undirected-graph", "Number of Connected Components", "Medium"],
    ["redundant-connection", "Redundant Connection", "Medium"],
    ["word-ladder", "Word Ladder", "Hard"],
  ]},
  { pattern: "Advanced Graphs", items: [
  ...
  { pattern: "1-D Dynamic Programming", items: [
  ...
  { pattern: "2-D Dynamic Programming", items: [
    ["unique-paths", "Unique Paths", "Medium"],
    ["longest-common-subsequence", "Longest Common Subsequence", "Medium"],
    ["best-time-to-buy-and-sell-stock-with-cooldown", "Buy and Sell Stock With Cooldown", "Medium"],
  ...
  { pattern: "Bit Manipulation", items: [
...
export const PATTERNS: string[] = GROUPS.map((g) => g.pattern);

export const NEETCODE_150: CurriculumProblem[] = GROUPS.flatMap((g) =>
  g.items.map(([slug, title, difficulty]) => ({
    slug, title, difficulty, pattern: g.pattern,
  }))
);

export const BY_PATTERN: Record<string, CurriculumProblem[]> = Object.fromEntries(
  GROUPS.map((g) => [
    g.pattern,
    g.items.map(([slug, title, difficulty]) => ({ slug, title, difficulty, pattern: g.pattern })),
  ])
);
